...
rpi.Time = {
  getTime: function (a, c, f) {
    return "chat" == f
      ? this.getChatTime(a, c || "en")
      : f
        ? this.getFormatTime(a, f, c || "en")
        : this.getDefaultTime(a, c || "en");
  },
  getChatTime: function (a, c) {
    var f = (new Date().getTime() - a) / 1e3 / 60 / 60,
      h = f / 24;
    return 24 > f
      ? this.getFormatTime(a, "HH:mm", c)
      : 365 > h
        ? this.getFormatTime(a, "dd.MM HH:mm", c)
        : this.getFormatTime(a, "yyyy.MM.dd HH:mm", c);
  },
  ...
  getTimeAgo: function (a, c) {
    ...
    var f = a / 60,
      ...
      e = h / 24,
      m = e / 365;
    c = rpi.Time.Messages[c] ? c : "en";
    return 45 > a
      ? rpi.Time.Messages[c].second
      : 90 > a
        ? rpi.Time.Messages[c].minute
        : 45 > f
          ? rpi.Time.Messages[c].minutes(f)
          : 90 > f
            ? rpi.Time.Messages[c].hour
            : 24 > h
              ? rpi.Time.Messages[c].hours(h)
              : 48 > h
                ? rpi.Time.Messages[c].day
                : 30 > e
                  ? rpi.Time.Messages[c].days(e)
                  : 60 > e
                    ? rpi.Time.Messages[c].month
                    : 365 > e
                      ? rpi.Time.Messages[c].months(e)
                      : 2 > m
                        ? rpi.Time.Messages[c].year
                        : rpi.Time.Messages[c].years(m);
  },
  ...
  getFormatTime: function (a, c, f) {
    var h = new Date(a),
      e = {
        ...
        HH: h.getHours(),
        hh:
          (h.getHours() % 12 ? h.getHours() % 12 : 12) +
          (12 <= h.getHours() ? "PM" : "AM"),
        dd: h.getDate(),
        MM: h.getMonth() + 1,
        yyyy: h.getFullYear(),
  ...
  declineNum: function (a, c, f, h) {
    return a + " " + this.declineMsg(a, c, f, h);
  },
...
rpi.Utils = {
  ...
  getParent: function (a, c) {
    ...
    if (0 > a.className.split(" ").indexOf(c))
      for (; (a = a.parentElement) && 0 > a.className.split(" ").indexOf(c); );
    return a;
  },
  lang: function () {
    var a = navigator;
  ...
  ajax: function (a, c, f, h) {
    ...
    e.onreadystatechange = function () {
      if (
        (e.readyState === XMLHttpRequest.DONE || e.readyState === e.DONE) &&
        200 === e.status
      ) {
        const m = JSON.parse(e.responseText);
        "function" === typeof h ? h(m) : "function" === typeof f && f(m);
      }
    };
  ...
  const h = a.getAttribute("data-id"),
    ...
    m = new RegExp(
      '$|(\\[data-id="' +
        ...
        '"\\]\\s+.' +
        f.card +
        "\\s*{\\s*--col:\\s*\\d+\\s*!important\\s*;?(\\s*--gap:\\s*\\d+\\s*!important;?)?})",
    ),
    l = a.getElementsByClassName(f.cnt)[0];
  var n = null;
  return (n = {
    init: function (k, q) {
      rpi.Utils.isVisible(l)
        ? (k && k(),
          n.resize(),
          window.addEventListener("resize", n.resize),
          q && q(),
          (rpi.Instances = rpi.Instances || []),
          rpi.Instances.push(n))
        : setTimeout(n.init, 300);
    },
    resize: function () {
      let k = n.getSize();
      l.className = l.className.replace(e, f.col + "-" + k);
      if (c.breakpoints) {
        let q = l.offsetWidth,
    ...
    getCol: function () {
      let k = this.getStyle().innerHTML.match(/--col:\s*(\d+)/);
      return k && 1 < k.length ? k[1] : !1;
    },
    getStyle: function () {
      let k = document.getElementById("rpi-style");
      k ||
    ...
    setBreakpoints: function (k) {
      c.breakpoints = k;
    },
    brsort: function (k, q) {
      return parseInt(k.split(":")[0]) > parseInt(q.split(":")[0]) ? 1 : -1;
    },
    clear: function () {
...
rpi.Common = function (a, c, f) {
  ...
  return (h = {
    init: function () {
      ...
      let m = a.getElementsByClassName(f.text);
      for (var l = 0; l < e.length; l++)
        e[l].innerHTML = h.time(e[l].getAttribute("data-time"));
      for (e = 0; e < m.length; e++)
        (l = m[e]) &&
          l.innerHTML &&
          ((l.innerHTML = h.trimtext(l.innerHTML)),
          (l = l.getElementsByClassName(f.readmore)[0])) &&
          (l.onclick = h.opentext);
    },
    initOnce: function (e) {
      var m = e.getElementsByClassName(f.time)[0];
      e = e.getElementsByClassName(f.text)[0];
      m && (m.innerHTML = h.time(m.getAttribute("data-time")));
      e &&
        e.innerHTML &&
        ((e.innerHTML = h.trimtext(e.innerHTML)),
        (m = e.getElementsByClassName(f.readmore)[0])) &&
        (m.onclick = h.opentext);
    },
    time: function (e) {
      return c.time_format
        ? e
        : rpi.Time.getTimeAgo(1e3 * parseInt(e), rpi.Utils.lang());
    },
    trimtext: function (e, m) {
      var l = parseInt(c.text_size);
      if (e && l && e.length > l) {
        var n = e.substring(0, l).indexOf(" ") + 1;
        if (1 > n || l - n > l / 2) n = l;
        var k = (l = "");
        0 < n &&
          ((l = e.substring(0, n - 1)), (k = e.substring(n - 1, e.length)));
        return (
          l +
          (!m && k
            ? '<span class="rpi-s">... </span><input type="hidden" value="' +
              encodeURIComponent(k) +
              '"></input><span class="' +
              f.readmore +
              '">' +
              rpi.Utils.__("read more", c.trans) +
              "</span>"
            : "")
        );
      }
    ...
    opentext: function () {
      let e = this.parentNode,
        m = this.previousSibling,
        l = this.previousSibling.previousSibling,
        n = decodeURIComponent(m.value);
      rpi.Utils.rm(m);
      rpi.Utils.rm(l);
      rpi.Utils.rm(this);
      e.innerHTML += n;
...
rpi.Slider = function (a, c, f, h) {
  ...
  const e = a.getElementsByClassName(f.cnt)[0],
    m = a.getElementsByClassName(f.content)[0],
    l = m.getElementsByClassName(f.cards)[0],
    n = m.getElementsByClassName(f.dotsWrap)[0],
    k = m.getElementsByClassName(f.dots)[0],
    q = parseInt(l.getAttribute("data-count"));
  var d = null,
    r = m.getElementsByClassName(f.card),
    u = "",
    C = "",
    y = null,
    w = null,
  ...
  return (d = {
    init: function (b, g) {
      rpi.Utils.isVisible(e)
        ? (b && b(),
          d.resize(),
          d.actions(),
          r.length && d.swipeAutoStart(),
          g && g(),
          (rpi.Instances = rpi.Instances || []),
          rpi.Instances.push(d))
        : setTimeout(d.init, 300);
    },
    resize: function (b) {
      let g = h.column.resize(),
        p = h.column.getCol();
      b &&
        l.scrollLeft != b * d.reviewWidth() &&
        l.scrollTo(r[b].offsetLeft, 0);
      !r.length ||
        (u == g && C == p) ||
        (d.hasDots() && (d.dotsInit(), d.dotSwipe(b, !0)), (u = g), (C = p));
    },
    actions: function () {
      c.mousestop && d.addMouseEvents();
      window.addEventListener("resize", d.resizeListener);
      l &&
        (l.addEventListener("scroll", d.scrollListener, !1),
        c.wheelscroll && m.addEventListener("wheel", d.wheelListener, !1));
      var b = e.getElementsByClassName(f.btnPrev)[0];
      b &&
        (b.onclick = function (g) {
          d.btnClick(-1);
        });
      if ((b = e.getElementsByClassName(f.btnNext)[0]))
        b.onclick = function (g) {
          d.btnClick(1);
        };
    },
    ...
    wheelListener: function (b) {
      ...
      if (
        (g =
          -1 < g.className.indexOf(f.text)
            ? g
            : -1 < g.parentNode.className.indexOf(f.text)
              ? g.parentNode
              : null) &&
    ...
    wheelEnd: function (b) {
      d.swipeManual(Math.sign(b.wheelDelta) * B * d.swipeStep());
      B = 0;
    },
    scrollEnd: function () {
      D = d.reviewsIdx();
      x ? (x = !1) : d.loadNextReviews();
      ((!c.mousestop || A) && c.mousestop) ||
        ((!c.clickstop || x) && c.clickstop) ||
        d.swipeAutoStart();
      d.hasDots() && d.dotSwipe(void 0, !0);
    },
    loadNextReviews: function (b) {
      b = b
        ? d.reviewsIdx() + parseInt(b)
        : d.hasDots()
          ? (b = k.getElementsByClassName("active")[0])
            ? parseInt(b.getAttribute("data-index")) * d.swipePerDot()
            : d.reviewsIdx()
          : d.reviewsIdx();
      b = d.getAjaxSize(b);
    ...
    getAjaxSize: function (b) {
      let g = 0;
      const p = parseInt(l.getAttribute("data-offset")),
        t = parseInt(c.pagination);
      if (q > p) {
        let E = b - p;
        Math.abs(E) < 3 * d.swipePerDot()
          ? (g = t)
          : E && (g = Math.ceil(b / t) * t - p);
      }
      b = p + g - q;
      return 0 < b ? g - b : g;
    },
    ...
    dotClick: function () {
      ...
      var g = k.getElementsByClassName("active")[0];
    ...
    dotSwipe: function (b, g) {
      ...
      b = 0 > b ? 0 : b >= k.childNodes.length ? k.childNodes.length - 1 : b;
      b = k.querySelector("." + f.dot + '[data-index="' + b + '"]');
      let p = k.getElementsByClassName("active")[0];
      d.dotActivate(p, b);
      d.dotScroll(b, g);
    },
    dotScroll: function (b, g) {
      let p = Math.round(k.scrollWidth / k.childNodes.length),
        t = Math.floor(Math.round(k.offsetWidth / p) / 2);
      g
        ? k.scrollTo(b.offsetLeft - t * p, 0)
        : k.scrollTo({ left: b.offsetLeft - t * p, behavior: "smooth" });
    },
    dotActivate: function (b, g) {
      b &&
        (b.classList.remove("active"),
        b.classList.remove("s1"),
        b.previousSibling &&
          (b.previousSibling.classList.remove("s2"),
          b.previousSibling.previousSibling &&
            b.previousSibling.previousSibling.classList.remove("s3")),
        b.nextSibling &&
          (b.nextSibling.classList.remove("s2"),
          b.nextSibling.nextSibling &&
            b.nextSibling.nextSibling.classList.remove("s3")));
      g &&
        (g.classList.add("active"),
        g.classList.add("s1"),
        g.previousSibling &&
          (g.previousSibling.classList.add("s2"),
          g.previousSibling.previousSibling &&
            g.previousSibling.previousSibling.classList.add("s3")),
        g.nextSibling &&
          (g.nextSibling.classList.add("s2"),
          g.nextSibling.nextSibling &&
            g.nextSibling.nextSibling.classList.add("s3")));
    },
    ...
    swipeAuto: function () {
      if (d.isScrollEnd()) d.scroll(-(q - d.reviewsPerView()));
        ...
        let b =
          d.swipeStep() < d.reviewsAhead() ? d.swipeStep() : d.reviewsAhead();
        d.scroll(b);
      }
      d.swipeAutoStart();
    },
    scroll: function (b) {
      ...
      g = 0 > g ? 0 : g >= r.length ? r.length - 1 : g;
      -1 < g &&
        g < r.length &&
        (l.scrollTo({ left: r[g].offsetLeft, behavior: "smooth" }),
    ...
    isScrollEnd: function () {
      var b = l.querySelector("." + f.card + ":last-child"),
        g = b.getBoundingClientRect();
      b = b.parentNode.getBoundingClientRect();
      return (
        (2 > Math.abs(b.left - g.left) || b.left <= g.left) &&
        g.left < b.right &&
        (2 > Math.abs(b.right - g.right) || b.right >= g.right) &&
        g.right > b.left
      );
    },
    swipeStep: function () {
    ...
    reviewWidth: function () {
      return Math.round(l.scrollWidth / r.length);
    },
    reviewHeight: function () {
      return r[0].offsetHeight;
    },
    reviewsPerView: function () {
      return Math.round(l.offsetWidth / d.reviewWidth());
    },
    reviewsIdx: function () {
      let b = rpi.Utils.isRTL() ? -l.scrollLeft : l.scrollLeft;
    ...
    hasDots: function () {
      return k && !c.hide_dots && 0 < d.swipePerDot();
    },
    ...
    clear: function () {
      ...
      l.removeEventListener("scroll", d.scrollListener);
      m.removeEventListener("wheel", d.wheelListener);
    },
  });
};
function rplg_badge_init(a, c, f) {
  ...
  h &&
    ...
    (a.className = f + " wpac"),
    -1 < h.className.indexOf("-fixed") && a.appendChild(h),
    a.appendChild(e),
...
function rplg_next_reviews(a, c) {
  ...
  for (var e = 0; e < c && e < reviews.length; e++)
    reviews[e] &&
      (reviews[e].className = reviews[e].className.replace(a + "-hide", " "));
  reviews = f.querySelectorAll(h);
...
function grw_init(a, c) {
  ...
  if ("true" != a.getAttribute("data-exec")) {
    ...
    rpi
      ...
      .init();
    !a.getElementsByClassName("grw-review")[0] ||
      ("slider" != c && "grid" != c) ||
      ((c = a.getElementsByClassName("grw-row")[0]),
      (c = JSON.parse(c.getAttribute("data-options"))),
      (f = rpi.Column(a, c, {
        cnt: "grw-row",
        col: "grw-row",
        card: "grw-review",
