      ...
      tabs.forEach((item) => {
        ...
        if (hasAnimation && animValue !== "" && animValue !== "true") {
          ...
          if (!isNaN(d) && !isNaN(o)) {
            ...
            console.log(`[Tabs] Invalid data-tabs-animate value: "${animValue}". Falling back to defaults.`);
          }
        }
        ...
        tabs_controlers.forEach((tab, idx) => {
          tab.addEventListener("click", (e) => {
            ...
            if (panel_prev_active) panel_prev_active.setAttribute("aria-selected", "false");

            let dft_panel = item.querySelector(`#${dft_tab__aria_controls}`);
            if (dft_panel) dft_panel.setAttribute("aria-selected", "true");
